...
* **Control de Permisos Granular (RBAC):** Sistema de roles estricto (`TWOOSTUDIO`, `ORG_ADMIN`, `USER`). El acceso a módulos sensibles como la galería de conceptos se controla individualmente por el administrador desde el panel de gestión en tiempo real.
* **Control Maestro Global (God Mode):** Panel de auditoría global exclusivo para administradores de sistema (`TWOOSTUDIO`). Permite la inspección profunda de organizaciones, lectura de archivos remotos, gestión de licencias globales, reubicación de usuarios y envío de notificaciones Broadcast de emergencia.
